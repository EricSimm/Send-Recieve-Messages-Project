...
void mainLoop()
{
    ...
    message recvMessage;
  	...
  	error = msgrcv(msqid, &recvMessage, sizeof(message) - sizeof(long), SENDER_DATA_TYPE, 0);
	if(error == -1)
   	{
   	...
   	cout << "msqid: " << msqid <<endl << "recvMessage: " << &recvMessage<<endl;  
    ...
    while(msgSize != 0)
    {
		...
		printf("Writing saved memory to file\n");
            
            //Sending message of type RECV_DONE_TYPE
            sendMessage.mtype=RECV_DONE_TYPE;
		printf("I am ready for the next chunk of data\n");
            
            //Sending message
            //Check SENDING message size by redefining msgSize
            error = msgsnd(msqid, &sendMessage, sizeof(sendMessage) - sizeof(long),0);
